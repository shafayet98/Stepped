...
public final class AddEditAlarmActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if(getSupportFragmentManager().findFragmentById(R.id.edit_alarm_frag_container) == null) {
            getSupportFragmentManager()
                    .beginTransaction()
                    .add(R.id.edit_alarm_frag_container, com.example.dell.alarmapp.Alarm.ui.AddEditAlarmFragment.newInstance(alarm))
                    .commit();
        }
    ...
    private com.example.dell.alarmapp.Alarm.model.Alarm getAlarm() {
        switch (getMode()) {
            ...
            case ADD_ALARM:
                final long id = com.example.dell.alarmapp.Alarm.data.DatabaseHelper.getInstance(this).addAlarm();
                LoadAlarmsService.launchLoadAlarmsService(this);
                return new com.example.dell.alarmapp.Alarm.model.Alarm(id);
            case UNKNOWN:
            default:
                throw new IllegalStateException("Mode supplied as intent extra for " +
                        com.example.dell.alarmapp.Alarm.ui.AddEditAlarmActivity.class.getSimpleName() + " must match value in " +
    ...
    public static Intent buildAddEditAlarmActivityIntent(Context context, @Mode int mode) {
        final Intent i = new Intent(context, com.example.dell.alarmapp.Alarm.ui.AddEditAlarmActivity.class);
